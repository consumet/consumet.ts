...
const checkLibgen = async (output: { output: string }) => {
  ...
  try {
    ...
    output.output += `| Libgen | ${status} | ${((t1 - t0) / 1000).toPrecision(3)} |\n`;

    const t2 = performance.now();
    let res = await axios.get(libgenDownload, { timeout: 3000 });
    const t3 = performance.now();
    output.output += `| Libgen Download | ${res.status} | ${(t3 - t2) / 1000} |\n`;

    return output.output;
  } catch (e: any) {
    if (e.config.url == libgenDownload) {
      output.output += `| Libgen Download | ${500} | 3 |\n`;
    }
  }
  return output.output;
};
...
const checkGetComics = async (output: { output: string }) => {
  ...
  try {
    ...
    const t1 = performance.now();
    output.output += `| GetComics | ${status} | ${((t1 - t0) / 1000).toPrecision(3)} |\n`;
  } catch (e: any) {
    output.output += `| GetComics | 500 | 3 |\n`;
  }
  return output.output;
};
