...
exports.extractMegaCloudSources = extractMegaCloudSources;
 ...
 */
function extractClientKey(html) {
    const patterns = [
        {
        ...
        {
            regex: /<script>window\._lk_db\s+=\s+\{x:\s*["']([a-zA-Z0-9]+)["'],\s*y:\s*["']([a-zA-Z0-9]+)["'],\s*z:\s*["']([a-zA-Z0-9]+)["']\};<\/script>/,
...
function decryptAES(encrypted, keyOrSecret) {
    ...
    return decipher.update(contents, undefined, 'utf8') + decipher.final('utf8');
}
 ...
 */
async function extractMegaCloudSources(videoUrl, referer) {
    ...
    const { data: embedHtml } = await axios_1.default.get(embedPageUrl, {
        headers: {
    ...
    const { data: srcsData } = await axios_1.default.get(getSourcesUrl, {
        headers: {
            Accept: '*/*',
    ...
    try {
        ...
        const { data: scriptText } = await axios_1.default.get(scriptUrl, {
...
//# sourceMappingURL=megacloud-util.js.map