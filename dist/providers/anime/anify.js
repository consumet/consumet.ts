...
class Anify extends models_1.AnimeParser {
    constructor() {
         ...
         * @param id anilist id
         */
        this.fetchAnimeInfoByAnilistId = async (id, providerId = 'gogoanime') => await this.fetchAnimeInfo(id, providerId);
        this.fetchEpisodeSources = async (episodeId, episodeNumber, id, providerId = 'gogoanime') => {
            const { data } = await this.client.get(`${this.baseUrl}/sources?providerId=${providerId}&watchId=${this.actions[providerId].format(episodeId)}&episodeNumber=${episodeNumber}&id=${id}&subType=sub&server=gogocdn`);
            return data;
        };
    }
    /**
