    ...
    try {
      ...
      if (streamUrl) {
        ...
        const token = $('script:contains("window.video")')
          .text()
          ?.match(/token': '(.*)'/)![1];
        const token360p = $('script:contains("window.video")')
          .text()
          ?.match(/token360p': '(.*)'/)![1];
        const token480p = $('script:contains("window.video")')
          .text()
          ?.match(/token480p': '(.*)'/)![1];
        const token720p = $('script:contains("window.video")')
          .text()
          ?.match(/token720p': '(.*)'/)![1];
        const token1080p = $('script:contains("window.video")')
          .text()
          ?.match(/token1080p': '(.*)'/)![1];
