import { IMangaChapter, IMangaChapterPage, IMangaInfo, IMangaResult, ISearch, MangaParser, MediaStatus } from "../../models";
...
class VyvyManga extends MangaParser {
    ...
    override search = async(query: string, page: number = 1): Promise<ISearch<IMangaResult>> => {
        ...
        try{
            ...
            const { data }: {data: string} = await this.client.get(`${this.baseWebsiteUrl}/search?search_po=0&q=${formattedQuery}&page=${page}`);

            const $: CheerioAPI = load(data);
            const dom = $('html');

            const result = dom.find(".row.book-list > div > div > a").map((index, ele) => {
                return {
                    id: ($(ele).find("div.comic-image").attr("data-background-image") as string).split("cover/")[1].split("/")[0],
            ...
            const lastPage = parseInt($(pagination[pagination.length - 2]).find("a").text().trim());
    ...
    override fetchMangaInfo = async (mangaId: string): Promise<IMangaInfo> => {
        try{
            ...
            const authors = $(dom.find(".col-md-7 > p:nth-child(2) > a")).map((index, ele) => $(ele).text().trim()).get()
            const status = ($(dom.find("div.col-md-7 > p")[1]).find("span:nth-child(3)").text().trim() as MediaStatus);
            const genres = $(dom.find("div.col-md-7 > p")[2]).find("a").map((index, ele) => $(ele).text().trim()).get();
            const description = dom.find(".summary > .content").text().trim();
            const chapters = dom.find(".list-group > a").map((index, ele) => {
    ...
    override fetchChapterPages = async(chapterId: string): Promise<IMangaChapterPage[]> => {
        try{
            ...
            const images: IMangaChapterPage[] = dom.find(".vview.carousel-inner > div > img").map((index, ele) => {
                return {
    ...
    private formatSearchResultData = (searchResultData: VyvyMangaSearchResultData[]): IMangaResult[] => {
        return searchResultData.map(ele => {
            return {
                id: `${ele.id}`,
                title: ele.name,
                altTitles: ele.title.split(",").filter(ele => ele.length).map(ele => ele.trim()),
