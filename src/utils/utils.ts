...
export const kitsuSearchQuery = (query: string) =>
  `query{searchAnimeByTitle(first:5, title:"${query}"){ nodes {id season startDate titles { localized } episodes(first: 1000){ nodes { number titles { canonical } description thumbnail { original { url } } } } } } }`;
