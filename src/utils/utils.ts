...
export const anilistAiringScheduleQuery = (page: number = 1, perPage: number = 20, greater: number , lesser: number, notYetAired: boolean = true) => 
  `query { Page(page: ${page}, perPage: ${perPage}) { pageInfo { total perPage currentPage lastPage hasNextPage } airingSchedules( notYetAired: ${notYetAired}, airingAt_greater: ${greater}, airingAt_lesser: ${lesser}) { airingAt episode media { id description idMal title { romaji english userPreferred native } bannerImage coverImage { extraLarge large medium color } genres averageScore seasonYear format } } } }`;
