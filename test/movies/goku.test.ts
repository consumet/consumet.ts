...
test('fetchMediaInfo: returns a filled object of movies data', async () => {
  const data = await goku.fetchMediaInfo('watch-movie/watch-batman-begins-19636');
  expect(data.description).not.toEqual('');
  expect(data.episodes).not.toEqual([]);
});

test('fetchEpisodeServers: returns a filled object of streaming movie servers', async () => {
  const data = await goku.fetchEpisodeServers('1064170', 'watch-movie/watch-batman-begins-19636');
  expect(data).not.toEqual([]);
});

test('fetchEpisodeSources: returns a filled object of streaming movie sources', async () => {
  const data = await goku.fetchEpisodeSources(
    '1613017',
    'watch-movie/watch-batman-begins-19636',
    StreamingServers.VidCloud
  );
  expect(data.sources).not.toEqual([]);
});

test('fetchMediaInfo: returns a filled object of tv data', async () => {
  const data = await goku.fetchMediaInfo('watch-series/watch-batman-39276');
  expect(data.description).not.toEqual('');
  expect(data.episodes).not.toEqual([]);
});

test('fetchEpisodeServers: returns a filled object of streaming tv servers', async () => {
  const data = await goku.fetchEpisodeServers('46259', 'watch-series/watch-batman-39276');
  expect(data).not.toEqual([]);
});

test('fetchEpisodeSources: returns a filled object of streaming tv sources', async () => {
  const data = await goku.fetchEpisodeSources(
    '46259',
    'watch-series/watch-batman-39276',
    StreamingServers.VidCloud
  );
  expect(data.sources).not.toEqual([]);
});

test('fetchRecentMovies: returns a filled object of recent movies', async () => {
  const data = await goku.fetchRecentMovies();
  expect(data).not.toEqual([]);
});

test('fetchRecentTvShows: returns a filled object of recent tv shows', async () => {
  const data = await goku.fetchRecentTvShows();
  expect(data).not.toEqual([]);
});

test('fetchTrendingMovies: returns a filled object of trending movies', async () => {
  const data = await goku.fetchTrendingMovies();
  expect(data).not.toEqual([]);
});

test('fetchTrendingTvShows: returns a filled object of trending tv shows', async () => {
